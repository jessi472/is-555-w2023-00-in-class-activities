...
tips %>% 
  ...
  ggplot(aes(x = total_bill, y = tip, color = sex, shape = smoker)) +
  geom_point(size = 2)+
  facet_wrap(smoker~size, nrow =2)# or vars(day) scales help with skewed data
#scales - fixed free, or free_x or free_y

tips %>% 
  mutate(tip_perc = tip/total_bill) %>% 
  ggplot(aes(x = total_bill, y = tip_perc, color = as.factor(size), shape = smoker)) +
  geom_point(size = 2)+
  facet_grid(smoker~day)# set number of tables and where located 
#wrap - doesn't show empty
